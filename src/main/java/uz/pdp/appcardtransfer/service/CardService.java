...
public class CardService {
    ...
    public List<Income> getIncomeHistory(Integer cardId) {
//        String token = request.getHeader("Authorization");
//        token = token.substring(7);
//        String username = jwtProvider.getUsernameFromToken(token);
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();

        return incomeRepository.findAllByToCardIdAndToCard_Username(cardId,user.getUsername());
    }


    public List<Outcome> getOutcomeHistory(Integer cardId) {
//        String token = request.getHeader("Authorization");
//        token = token.substring(7);
//        String username = jwtProvider.getUsernameFromToken(token);
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        return outcomeRepository.findAllByFromCardIdAndFromCard_Username(cardId,user.getUsername());
    }
}
